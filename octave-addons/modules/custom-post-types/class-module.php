...
class Octave_Addons_Module_Custom_Post_Types extends Octave_Addons_Module {
	...
	protected function render_field_card( string $index, array $field, array $post_types, bool $saved, string $primary_post_type = '' ): void {
		...
		<article class="oa-cpt-item oa-collection-item oa-field-scope--<?= esc_attr( $scope ); ?>"<?= '' !== $name ? ' id="oa-field-' . esc_attr( $name ) . '"' : ''; ?> data-saved="<?= $saved ? 'true' : 'false'; ?>">
			...
			<div class="oa-cpt-item-head">
				...
				<button type="button" class="oa-cpt-expand oa-collection-expand" aria-expanded="<?= $saved ? 'false' : 'true'; ?>"><span class="oa-cpt-expand-copy"><strong class="oa-cpt-item-title"><?= esc_html( '' !== $label ? $label : __( 'New post field', 'octave-addons' ) ); ?></strong><span class="oa-field-type-preview"><?= esc_html( $types[ $type ] ?? __( 'Field', 'octave-addons' ) ); ?></span><span class="oa-field-scope-badge" data-used-label="<?php esc_attr_e( 'Reusable · Used here', 'octave-addons' ); ?>" data-available-label="<?php esc_attr_e( 'Reusable · Available', 'octave-addons' ); ?>"><?= esc_html( $scope_label ); ?></span></span><span class="dashicons dashicons-arrow-down-alt2 oa-cpt-expand-icon" aria-hidden="true"></span></button>
	...
	protected function normalise_fields( $fields, array $post_types ): array {
		...
		foreach ( array_slice( $fields, 0, 50 ) as $index => $field ) {
			...
			} elseif ( in_array( $type, [ 'wysiwyg', 'html' ], true ) ) {
	...
	-- Lists every editor-visible post type currently registered on the site,
	-- together with authors as the supported non-post relationship source.
	---------------------------------------------------------- */

	protected function reference_source_options(): array {
		...
		natcasesort( $options );
		$options['author'] = __( 'Authors', 'octave-addons' );
	...
	protected function sub_field_types(): array {
		...
		return array_diff_key( $this->field_types(), array_flip( [ 'group', 'repeater', 'html', 'tab', 'cpt_select' ] ) );
